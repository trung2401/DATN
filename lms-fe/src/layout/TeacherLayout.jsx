...
const TeacherLayout = ({ children }) => {
  ...
  const handleLogout = async () => {
    ...
    const res = await authLogout({ accessToken, refreshToken });
      ...
      <div className="flex flex-row">
        <aside className="w-64 border-r-2 border-gray-200 p-4 space-y-4 sticky top-0 h-screen overflow-y-auto scroll-smooth">
          ...
          <nav className="flex flex-col space-y-2">
            ...
            <NavLink to="/teacher/exam" className={linkClass}>
              <span className="flex gap-4 items-center">
                <FontAwesomeIcon icon="fa-solid fa-file" className="w-5 h-5" />
                Quản lý đề thi
              </span>
            </NavLink>
            <NavLink to="/teacher/course" className={linkClass}>
              <span className="flex gap-4 items-center">
                <FontAwesomeIcon icon="fa-solid fa-graduation-cap" className="w-5 h-5" />
                Quản lý khóa học
              </span>
            </NavLink>
            <NavLink to="/teacher/vocabulary" className={linkClass}>
              <span className="flex gap-4 items-center">
